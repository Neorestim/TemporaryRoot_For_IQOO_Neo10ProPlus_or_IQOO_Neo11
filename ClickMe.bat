@echo off
:: ===== Ctrl+C 保护：外层无限循环 =====
:: 按 Ctrl+C 后选 Y 终止内层，外层会重新启动脚本回到主菜单
if "%~1"=="__INNER__" goto REAL_START
:WRAPPER_LOOP
cmd /c "%~f0" __INNER__
goto WRAPPER_LOOP
...
cls
echo =============================================================
echo   适用于 IQOO Neo10 Pro+ / IQOO Neo11 的一键临时root工具 v2.1
echo =============================================================
echo.
...
echo [4] 退出
echo.
echo 提示：任何等待阶段按 Ctrl+C 后选择 Y 可返回主菜单。
echo.
set "USER_CHOICE="
...
set "ROOT_BAT=%temp%\root_exec_%random%.bat"
del "%ROOT_FLAG%" 2>nul

(
    echo @echo off
    echo set "PATH=%ADB_DIR%;%%PATH%%"
    echo adb shell "LD_PRELOAD=/data/local/tmp/preload.so /system/bin/id"
    echo if not errorlevel 1 echo OK^>"%ROOT_FLAG%"
) > "%ROOT_BAT%"

start "" /min /wait cmd /c "%ROOT_BAT%"
...
set "ROOT_RETRY_BAT=%temp%\root_retry_exec_!random!.bat"
...
(
    ...
    echo set "PATH=%ADB_DIR%;%%PATH%%"
    echo adb shell "LD_PRELOAD=/data/local/tmp/preload.so /system/bin/id"
    echo if not errorlevel 1 echo OK^>"!ROOT_RETRY_FLAG!"
) > "!ROOT_RETRY_BAT!"

start "" /min /wait cmd /c "!ROOT_RETRY_BAT!"
